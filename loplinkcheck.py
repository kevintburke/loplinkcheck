...
##NEED TO CLEAN UP OUTPUT, ADD COLUMN HEADERS, ADD PRINT STATEMENTS TO TERMINAL TO TRACK PROGRESS, AND ADD A WARNING THAT PROCESS WILL RUN IN BACKGROUND AND MAY TAKE SOME TIME TO COMPLETE, DEPENDING ON THE NUMBER OF LINKS BEING RUN
...
class LinkChecker:
    def __init__(self, root):
        ...
        tk.Checkbutton(root, text="Proxy links for testing (OpenAthens)?", variable=self.proxyyn).pack(pady=10)
    ...
    def checkurls(self):
        ...
        output = filedialog.asksaveasfilename(filetypes={("CSV",".csv")})
        ...
        for url in urls:
            ...
            for i in range(7):
                ...
                try:
                    ...
                    self.prog.update_idletasks()
                    break
            if status and (status.status_code not in [200, 404]):
                url.append(f'{status.status_code}: {status.reason}')
